...
I think that building authorization and API access control can help to mitigate current API security issues that can be present in complex multi-tenant SaaS applications. For more information, check out this [guide by AWS](https://docs.aws.amazon.com/prescriptive-guidance/latest/saas-multitenant-api-access-authorization/welcome.html) on the topic. AWS also has its own authorization policy language specifically tailored for application development, called [Cedar](https://github.com/cedar-policy/cedar-examples/tree/release/3.1.x/cedar-example-use-cases). Cedar can express RBAC, ABAC, and ReBAC type policies. One of the language's highlights is its policy validation and analysis capabilities. Using SMT solving under the hood, Cedar can formally reason about the policies. For example, it is able to proof that after refactoring a set of policies, the authorized permissions do not change.
